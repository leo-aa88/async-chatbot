...
- **A focus-setting turn that produces a provisional memory** sets the focus to that memory id — a new subject, regardless of prior mode. In v0.7 a turn is *confidently* focus-setting only via a **structural cue** — a task/imperative, a re-prompt, or a question (including a short one, "what about a coma?"); shortness is not contentlessness. A plain declarative statement is **not** treated as focus-setting (see the realization note below).
...
**Realization: focus-setting in v0.7 is a *structural cue*, not the ingress length class or a word list.** A task/imperative, re-prompt, or question is an affirmative, deterministic signal that a new subject was asserted. A **plain declarative statement is deliberately not focus-setting**, because acknowledgement-vs-subject in a bare declarative ("I hear you loud and clear" vs "The robot needs better balance") is not deterministically decidable: the ingress class is only a length split (`HIGH_INFORMATION`/`STATEMENT`), `ok`/`lol` make no memory, `noted`/`alright` are `STATEMENT`/`RAW`, and no acknowledgement word list closes the gap (the next unlisted ack always leaks). Per the uncertainty policy, the whole declarative class therefore falls to the *uncertain* branch and does not change the subject. Provisional-memory existence is not proof of substance — the predicate decides, and a memory only supplies the id once it has said focus-setting.

**Accepted v0.7 limitation (this is the contract, not a hidden gap).** Because plain declaratives do not anchor the focus, a purely declarative first subject or topic shift is not caught by the gate: with no prior focus it **fails open** (the candidate is allowed), and mid-conversation it **keeps the prior subject** (which may over-suppress an on-topic candidate for the new, undetected subject — the §34.4 over-suppression failure mode). This is the price of never installing an acknowledgement as the subject. In practice the focus is anchored whenever the human asks a question or gives a task, which covers the motivating trace (a question-driven discussion). Robust declarative-subject detection needs an LLM-*proposed* relation (deterministically gated, invariant 5) and is deferred to §31.15.

**The other direction is largely closed by the structural rule:** an acknowledgement is installed as the focus only if the ingress classifier itself miscalls it a task/question (e.g. "you get it?"), not because a declarative ack slipped a word list. Both directions remain regression cases (§34.10); threshold/behaviour calibration and the LLM refinement are §31.15.
...
5. **A question sets the focus; acknowledgements and plain declaratives do not.** A structural cue (`"what about the robots?"`) becomes the focus; `noted`/`alright`, an unlisted multiword ack (`"I hear you loud and clear"`), and an ordinary declarative (`"The robot needs better balance."`) do **not** — the next `IDLE` wake is scored against the prior substantive subject, never the acknowledgement or the undetected declarative.
6. **Accepted v0.7 declarative limitation (documented contract).** A purely declarative first subject leaves the focus unset (gate fails open); a declarative topic shift mid-conversation keeps the prior subject (may over-suppress an on-topic candidate for the new subject). Both are asserted through `handle_human_message` so the contract is pinned, not silently regressed.
