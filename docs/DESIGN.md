...
- a *question or task* usually introduces a subject, but a **confirmation/check-in question** ("Is that clear?", "make sense?") and a `REPROMPT` ("You there?") establish only *response obligation*. `REPROMPT` is excluded outright; a confirmation question is a **known residual** that can still be installed as the focus.
...
**Accepted v0.7 limitations (these are the contract, not hidden gaps).** (1) A purely declarative first subject or topic shift is not caught: with no prior focus the gate **fails open** (candidate allowed); mid-conversation it **keeps the prior subject** (may over-suppress an on-topic candidate for the new, undetected subject). (2) A confirmation/check-in question can become the focus, over-suppressing real candidates for one `IDLE` band. Both residuals degrade toward **silence/over-suppression**, never toward voicing an orphan, and reset on the next real subject turn. In practice the focus is anchored whenever the human asks a substantive question or gives a task (the motivating trace is question-driven). A faithful predicate needs an LLM-*proposed* relation (deterministically gated, invariant 5) and is deferred to §31.15; the deterministic v0.7 gate is intentionally narrow. Regression cases pin both the working paths and these documented residuals (§34.10).
