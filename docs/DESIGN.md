...
`focus_memory_id is None` was overloaded across three situations that are **not** behaviorally equivalent — *never had a focus* (a fresh conversation), *deliberately closed* (the human resolved/dropped the thread), and *lapsed* (retired after dormancy, §34.4). The gate failed open on all three, so a **just-closed** thread got renagged: in `IDLE`, with no focus, a candidate from the resolved thread was voiced. The continuity corpus (§27) surfaced this as an `intrusion` — and correctly diagnosed it as a *representational* hole, not a threshold: the state model could not say *why* the focus was absent.

A single flag closes it: **`ConversationState.subject_closed`**, set when a §35.3 `CLEAR` resolves the thread. The affinity gate (§34.5) gains one case — while `IDLE` **and** `focus_memory_id is None`, a candidate is muted iff `subject_closed` (don't renag a closed thread while the human is present), and otherwise still fails open (the declarative residual, case 6). It is **deliberately `IDLE`-scoped**: `DORMANT` leaves the gate inactive, so a later lull still resurfaces (§34.7) — closing a thread quiets it *now*, it does not forbid the thought forever. The marker is transient: it clears the moment a new subject is established (any focus write to a non-`None` id) or a lull ends the just-closed window (a turn whose pre-mode is `DORMANT`), and it is **held across a bare acknowledgement** so an `ok` can't silently re-open a resolved thread. The deterministic path never *sets* it — closing is an LLM-proposed judgement (`CLEAR`, §35.3), so a no-call closer is the same documented residual as any other no-call turn.

**Regression cases (v0.8):** (a) `IDLE` + no focus + `subject_closed` ⇒ candidate muted; (b) `IDLE` + no focus + not closed ⇒ fail open (case 6 preserved); (c) `DORMANT` + `subject_closed` ⇒ speaks (resurfacing preserved); (d) a `CLEAR` result sets `subject_closed`; (e) a focus-setting turn clears it; (f) a bare ack holds it; (g) a turn after a lull clears it.
...
- **`CLEAR`** — focus becomes none **and the subject is marked closed** (`subject_closed`, §34.11): the human resolved/dropped the thread. While `IDLE` the gate then declines to renag that closed thread (it does *not* fail fully open as a bare absent focus would); the marker clears once a new subject is set or a lull ends, and `DORMANT` resurfacing is untouched. (A widening for *unrelated* candidates, a restraint for the *closed* one — §35.6.)
