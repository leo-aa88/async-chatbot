...
**Backchannel vs focus-setting is its own small, conservative classification — not the existing task/social class or memory status.** The current ingress signals are insufficient and must not be reused as-is: `ok`/`lol` are trivial and create *no* provisional memory, while `noted`/`alright` are classified `STATEMENT` (salience 0.5, a `RAW` memory) — none are `DO_NOT_ENRICH`. An implementation that keyed "backchannel" off memory status or the ack-token set would let `noted`/`alright` become the focus and score the next `IDLE` wake against "noted" instead of the real subject. The v0.7 predicate is a dedicated conservative check. Under the unified rule above a misclassification is bounded either way: a contentless turn wrongly called "focus-setting" produces no new subject-memory (or, if it did, the text really was substantive), and a substantive turn wrongly called "backchannel" merely defers to mode instead of updating the subject — neither revives a stale subject across dormancy. The tokens above are explicit regression cases (§34.10).
