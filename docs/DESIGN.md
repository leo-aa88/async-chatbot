...
### 34.11 A closed subject is not "no subject yet" (v0.8)

`focus_memory_id is None` was overloaded across three situations that are **not** behaviorally equivalent — *never had a focus* (a fresh conversation), *deliberately closed* (the human resolved/dropped the thread), and *lapsed* (retired after dormancy, §34.4). The gate failed open on all three, so a **just-closed** thread got renagged: in `IDLE`, with no focus, a candidate from the resolved thread was voiced. The continuity corpus (§27) surfaced this as an `intrusion` — and correctly diagnosed it as a *representational* hole, not a threshold: the state model could not say *why* the focus was absent, nor *which* subject had closed.

The fix keeps that **identity**: **`ConversationState.closed_focus_memory_id`** — the provisional-memory id a §35.3 `CLEAR` most recently closed, distinct from the current focus (a closed subject is no longer the focus). While `IDLE` with no active focus, the affinity gate (§34.5) **inverts** against it — the exact mirror of the open-focus rule:

```
OPEN focus:    related (CONTINUE/BRIDGE) → allowed,  unrelated (ORPHAN) → muted
CLOSED subject: related (CONTINUE/BRIDGE) → muted,   unrelated (ORPHAN) → allowed
```

So closing subject X renags **only X** — a candidate about X stays silent (don't reopen a resolved thread) while a genuinely unrelated worthwhile thought still speaks. A boolean would have muted *everything* in the `IDLE` window, trading one corpus cell for an unmeasured suppression elsewhere; the identity is what lets the gate say "not *that* thread" rather than "nothing at all". It is **deliberately `IDLE`-scoped**: `DORMANT` leaves the gate inactive, so a later lull resurfaces even X (§34.7) — closing quiets a thread *now*, it does not forbid the thought forever. The marker is transient: it clears the moment a new subject is established (any focus write to a non-`None` id) or a lull ends the just-closed window (a turn whose pre-mode is `DORMANT`), and is **held across a bare acknowledgement** so an `ok` can't silently reopen a resolved thread. The deterministic path never *sets* it — closing is an LLM-proposed judgement (`CLEAR`, §35.3), so a no-call closer is the same documented residual as any other no-call turn. `UNJUDGED` (no closed-subject vector, cross-model) fails open, like the rest of §34.5.

**Regression cases (v0.8):** (a) `IDLE` + no focus + closed subject + *related* candidate ⇒ muted; (b) `IDLE` + closed subject + *unrelated* candidate ⇒ **speaks** (renag only the closed thread — the anti-over-suppression guard); (c) `IDLE` + nothing closed ⇒ fail open (case 6 preserved); (d) `DORMANT` + closed subject ⇒ speaks (resurfacing preserved); (e) a `CLEAR` records the closed id; (f) a focus-setting turn clears it; (g) a bare ack holds it; (h) a turn after a lull clears it.
...
- **`KEEP`** — the subject is unchanged **from before this turn** (a check-in, an acknowledgement-with-a-question, an elaboration). Because the transition *overrides* the deterministic provisional, `KEEP` reverts the focus to the **pre-turn** subject — so an unlisted check-in that §34.4 wrongly anchored to this turn's memory is corrected, not merely left on the wrong anchor (this is how `KEEP` "fixes the check-in residual"). The handler recovers the pre-turn focus from the human turn's own work snapshot (`source.prior_focus_memory_id`, recorded at reduction).
...
- **`CLEAR`** — focus becomes none **and the closed subject's identity is retained** (`closed_focus_memory_id`, §34.11): the human resolved/dropped the thread. While `IDLE` the gate then renags *only that thread* — a candidate related to it is muted, a genuinely unrelated one still speaks (it does *not* fail fully open as a bare absent focus would, nor mute everything as a boolean flag would); the marker clears once a new subject is set or a lull ends, and `DORMANT` resurfacing is untouched. (A widening for *unrelated* candidates, a restraint for the *closed* one — §35.6.)
...
- **`ORPHAN` mutes only while a live thread actually exists — mode `IDLE` *and* a focus subject set (`focus_memory_id is not None`).** "Off the current thread" is meaningful only against a *current* subject, so both parts are load-bearing: in `DORMANT` there is no live window, and in `IDLE` *with no focus* (only check-ins were ever exchanged, so no turn set the focus) there is no thread either — in both, an `ORPHAN`-labelled message is a legitimate **resurfacing**, which §34.7 deliberately preserves, so the model's label must not suppress it. The guard is focus *existence*, not its vector: an LLM semantic judgement must not hinge on whether an embedding happened to be available — that cosine fail-open is §34.5's concern (and there the deterministic §34 gate already fails open with no focus), not this one. (An earlier draft muted `ORPHAN` in any mode; that would let the label kill dormant resurfacing, one of ACA's distinctive behaviors. A subsequent draft keyed only on mode `IDLE`; that still over-suppressed the reachable IDLE-but-no-focus state.)
...
1. **Suppress by kind; forward moves pass.** `REPEAT` is dropped at pre-outbox in any mode; `ORPHAN` is dropped **only while a live thread exists** — `IDLE` *and* a focus subject set; a forward move (`ADVANCE`/`EVIDENCE`/`REVISE`/`CLOSE`) leaves the §34 decision untouched (it speaks iff §34 already allows).
2. **`ORPHAN` does not suppress a resurfacing with no live thread.** A proactive result labeled `ORPHAN` still speaks when there is no current thread to be off — either `DORMANT` (no live window) or `IDLE` with `focus_memory_id is None` (only check-ins exchanged, no focus ever set). This is the resurfacing §34.7 preserves. (The load-bearing guard: an `ORPHAN`-in-any-mode gate, *or* one keyed on mode `IDLE` alone, would silence it.)
...
5. **`KEEP` / `REPLACE` / `CLEAR` effect.** On a call-making human-turn *result* (a mandatory *satisfied* speak, or a reactive reply that was **not** superseded — a stale/superseded result never writes the focus): `KEEP` reverts the focus to the **pre-turn** subject (overriding the deterministic provisional, so an unlisted check-in's mis-anchor is corrected); `REPLACE(valid memory id)` sets the focus to that memory; `CLEAR` sets it to none — after which the next `IDLE` wake **fails the gate open** (a candidate the old focus would have `ORPHAN`ed can now speak), which is a *deliberate widening*, not a suppression (mirrors §34.10 case 4's deterministic `DORMANT` clear).
